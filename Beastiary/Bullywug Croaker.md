A type of [[Bullywug]].
```statblock
name: Bullywug Croaker
size: Medium
type: humanoid
subtype: bullywug
alignment: neutral evil
ac: 15
hp: 33
hit_dice: 6d8+6
speed: 20 ft., swim 40 ft.
stats: [14, 12, 13, 7, 14, 8]
skillsaves:
  - perception: 4
  - stealth: 3
senses: passive Perception 14
languages: Bullywug
source: "Ghosts of Saltmarsh"
cr: 1
traits:
  - name: Amphibious
    desc: "The bullywug can breathe air and water."
  - name: Speak with Frogs and Toads
    desc: "The bullywug can communicate simple concepts to frogs and toads when it speaks in Bullywug."
  - name: Standing Leap
    desc: "The bullywug's long jump is up to 20 feet and its high jump is up to 10 feet, with or without a running start."
  - name: Swamp Camouflage
    desc: "The bullywug has advantage on Dexterity (Stealth) checks made to hide in swampy terrain."
actions:
  - name: Multiattack
    desc: "The bullywug croaker makes two attacks: one with its bite and one with its spear."
  - name: Bite
    desc: "Melee Weapon Attack: +4 to hit, reach 5 ft., one target. Hit: 4 (1d4 + 2) bludgeoning damage."
  - name: Spear
    desc: "Melee or Ranged Weapon Attack: +4 to hit, reach 5 ft. or range 20/60 ft., one target. Hit: 5 (1d6 + 2) piercing damage, or 6 (1d8 + 2) piercing damage if used with two hands to make a melee attack."
  - name: Croaking Chorus
    desc: "The croaker can use an action to let out a thunderous croak. Each creature within 10 feet that can hear the croaker must succeed on a DC 11 Constitution saving throw or become frightened until the end of the croaker's next turn."
```
